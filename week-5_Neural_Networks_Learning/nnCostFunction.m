...
y1=eye(num_labels,num_labels-1);
y1=[[zeros(num_labels-1,1);1],y1];
...
Y2=repelem(y1,1,count);
sum_1=sum(sum(-(Y2.*log_h)-((1-Y2).*log_h2)));
J=sum_1/m;

regterm=sum(sum(Theta1(:,2:end).^2,2))+sum(sum(Theta2(:,2:end).^2,2));
full_regterm=(regterm*lambda)/(2*m);
J=J+full_regterm;

delta2=zeros(size(Theta2));
delta1=zeros(size(Theta1));
del3=h3-Y2;
del2=(Theta2'*del3).*sigmoidGradient([ones(1,m);Z2]);
del2=del2(2:end,:);
